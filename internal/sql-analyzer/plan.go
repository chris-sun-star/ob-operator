...
func (c *PlanCollector) CollectAsync(audits []SQLAudit) {
	c.mu.Lock()
	defer c.mu.Unlock()
	logger.Printf("Collecting plans for %d audit records", len(audits))
	newPlanCount := 0
	...
	logger.Printf("Found %d new plans", newPlanCount)
}
...
func (w *PlanWorker) Start(ctx context.Context) {
	...
	for {
		...
		case ident := <-w.inputChan:
			logger.Printf("Fetching plan for tenant %d, server %s, port %d, plan %d", ident.TenantID, ident.SvrIP, ident.SvrPort, ident.PlanID)
